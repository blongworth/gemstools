# Functions for reading and parsing raw GEMS data


#' GEMS raw file parser
#'
...
#'
#' @param df_raw a raw GEMS dataframe
#'
#' @return a dataframe with added metadata
#' @export
gems_add_metadata <- function(df_raw) {
  df_raw |>
    mutate(
      row_num_i = dplyr::row_number(),
...
#' parse GEMS post times
#'
#' @param df a GEMS dataframe with added metadata
#'
#' @return a dataframe of post times
#' @export
gems_post_times <- function(df) {
  df |>
    dplyr::filter(type == "$") |>
    mutate(
      timestamp = lubridate::ymd_hms(data),
...
gems_turbo_status <- function(df) {
  df |>
    ...
    dplyr::mutate(
      ...
      filament = as.numeric(filament)
    ) # Amps
}

#' parse GEMS status data
#'
#' @param df a GEMS dataframe with added metadata
#'
#' @return a dataframe of status data
#' @export
gems_status_data <- function(df) {
  df |>
    dplyr::filter(type == "S") |>
    tidyr::separate(data,
      into = c(
...
#'
#' @export
gems_clean_adv_data <- function(adv) {
  adv |>
    filter(
      !is.na(count),
      count >= 0,
...
#' previous to current
#' @export
gems_missing <- function(count, line = NULL) {
  missing <- dplyr::case_when(
    count > 255 | dplyr::lag(count) > 255 ~
      NA_integer_,
...
#' @export
read_par_odyssey <- function(file) {
